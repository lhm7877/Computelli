...
cu.setPackageName("addition");
        ...
        MethodDeclaration method = type.addMethod(className+"_"+ propertyTypeAndName[1], Modifier.PUBLIC, Modifier.STATIC);
...
"System.out.println(refText + crfModelPath);\n"+
			className+"."+className+"(refText);\n"
			);
        method.setBody(block);
		
		type.addMember(field2);
		type.setName(className + "_" + propertyTypeAndName[1]);
		ConnectionAlgoDB.updateDB("algorithm", className + "_" + propertyTypeAndName[1],className + "_" + propertyTypeAndName[1], inArgType, outArgType, cu.toString(),numInArg,operSymbol,className+"_"+ propertyTypeAndName[1]);
	}
}
